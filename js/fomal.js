...
function showWelcome() {
  ...
  let dist = getDistance(121.526077, 31.259541, ipLoacation.result.location.lng, ipLoacation.result.location.lat); //这里换成自己的经纬度
    ...
    case "中国":
      ...
      switch (ipLoacation.result.ad_info.province) {
        ...
        case "天津市":
          ...
          break;
        case "重庆市":
          posdesc = "我希望遇见一个如你一般的人，如山间清爽的风，如古城温暖的光，只要最后是你就好。如果再来一次，还是重庆，因为，在这里遇见的她。";
          break;
        case "河北省":
  ...
  let timeChange;
  let date = new Date();
  if (date.getHours() >= 5 && date.getHours() < 8) timeChange = "<span>早安</span>，一日之计在于晨！";
  else if (date.getHours() >= 8 && date.getHours() < 11) timeChange = "<span>上午好</span>，没事多运动哈。";
  ...
  try {
    ...
    document.getElementById("welcome-info").innerHTML =
      `<b><center>🎉 欢迎信息 🎉</center>&emsp;&emsp;欢迎来自 <span style="color:var(--theme-color)">${pos}</span> 的小伙伴，${timeChange}您现在距离马尔克约 <span style="color:var(--theme-color)">${dist}</span> 公里，当前的IP地址为： <span style="color:var(--theme-color)">${ip}</span>， ${posdesc}</b>`;
  } catch (err) {
...
function createtime1() {
  var grt = new Date("12/25/2024 00:00:00"); //此处修改你的建站时间或者网站上线时间
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| mark") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('mark的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
if (m == 6 && dd == 20) {//站长生日
  if (sessionStorage.getItem("isPopupWindow") != "1") {
    Swal.fire("祝马尔克" + (y - 2003).toString() + "岁生日快乐！🥝");
    sessionStorage.setItem("isPopupWindow", "1");
  }
}
if (m == 6 && dd == 30) {//小猫咪生日
...
if ((lunar["IMonthCn"] == "五月" && lunar["IDayCn"] == "二十一")) {
  if (sessionStorage.getItem("isPopupWindow") != "1") {
    Swal.fire("祝马尔克生日快乐\n🎂🎂🎂");
    sessionStorage.setItem("isPopupWindow", "1");
  }
...
if ((lunar["IMonthCn"] == "七月" && lunar["IDayCn"] == "初九")) {
  if (sessionStorage.getItem("isPopupWindow") != "1") {
    Swal.fire("生日快乐\n对我的女孩😘");
    sessionStorage.setItem("isPopupWindow", "1");
  }
...
class Cursor {
  ...
  create() {
    ...
    //修改鼠标样式颜色
    this.scr.innerHTML = `* {cursor: url("data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 8 8' width='8px' height='8px'><circle cx='4' cy='4' r='4' opacity='1.0' fill='rgb(18, 150, 232)'/></svg>") 4 4, auto}`;
  }
...
function createtime() {
  ...
  var start = new Date("04/11/2024 00:00:00"); // 旅行者1号开始计算的时间
  var dis = Math.trunc(23400000000 + ((now - start) / 1000) * 17); // 距离=秒数*速度 记住转换毫秒
  var unit = (dis / 149600000).toFixed(6);  // 天文单位
  // 网站诞生时间
  var grt = new Date("04/11/2024 00:00:00");
...
if (localStorage.getItem("blogbg") != undefined) {
  ...
  document.getElementById("defineBg").innerText = `:root{
    --default-bg: url(https://image.cdn2.seaart.ai/2023-08-24/15209028855554053/a5241d5a14b621d73439a5181dd2ee07f97fc1c8_high.webp);
    --darkmode-bg:url(https://cdn.pixabay.com/photo/2023/11/29/03/40/ai-generated-8418596_1280.jpg);
    --mobileday-bg: url(https://cdn.pixabay.com/photo/2023/11/29/03/40/ai-generated-8418596_1280.jpg);
    --mobilenight-bg: url(https://cdn.pixabay.com/photo/2023/03/01/01/49/mountain-7822071_1280.jpg;
  }`;
}
// 切换背景主函数
function changeBg(s) {
